from app_R_K.excel_validation.utilits import (comparison_of_dictionaries,
                                              upd_to_float_dict_1,
                                              upd_to_float_dict_2)
...
def get_hours_list():
    ...
    basic_record_from_base = BasicTypeGraphic.query.with_entities(
        BasicTypeGraphic.main_date,
        BasicTypeGraphic.hours).all()
    ...
    for row in range(22, 632, 4):
        ...
        hours_2 = sheet.iter_rows(
            min_row=row + 2,
            max_row=row + 2, min_col=4,
            max_col=19,
            values_only=True
            )
